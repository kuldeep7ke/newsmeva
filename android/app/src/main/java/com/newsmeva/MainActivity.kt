...
class MainActivity : AppCompatActivity() {
    ...
    private lateinit var btnTeleprompter: MaterialButton
    private lateinit var btnScriptEditor: MaterialButton
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnTeleprompter = findViewById(R.id.btnTeleprompter)
        btnScriptEditor = findViewById(R.id.btnScriptEditor)
    ...
    private fun setupMiniApps() {
        btnTeleprompter.setOnClickListener { openTeleprompter() }
        btnScriptEditor.setOnClickListener { openScriptEditor() }

        listOf(btnTeleprompter, btnScriptEditor).forEach { btn ->
            btn.setOnTouchListener { v, event ->
                when (event.action) {
                    android.view.MotionEvent.ACTION_DOWN -> {
                        v.animate().scaleX(0.95f).scaleY(0.95f).setDuration(100).start()
    ...
    private fun openTeleprompter() {
        val intent = android.content.Intent(this, MiniAppActivity::class.java).apply {
            putExtra(MiniAppActivity.EXTRA_APP_TYPE, MiniAppActivity.APP_TYPE_TELEPROMPTER)
        }
        startActivity(intent)
    }

    private fun openScriptEditor() {
        val intent = android.content.Intent(this, MiniAppActivity::class.java).apply {
            putExtra(MiniAppActivity.EXTRA_APP_TYPE, MiniAppActivity.APP_TYPE_SCRIPT_EDITOR)
        }
        startActivity(intent)
    }
}
