...
| Ubuntu/RHEL systemd | `systemctl start newsmeva` — `Restart=always` + `Wants=caddy.service` | `systemctl stop newsmeva` | enabled by `install.sh` |
...
| 77 | **Ads: Brand placement + sub-types 2026-09-02** - new "Brand" option added to the ad Placement dropdown; selecting it reveals a required **Brand Type** sub-dropdown (Laptop Branding, Logo Branding, Sponsor, Special Program, Other). Slots and Duration are hidden in the form and on the ad card when Brand is chosen. New `ads.brand_type` column added to Postgres + SQLite schemas and migration fallback; create/update routes persist it (only for `ad_place='brand'`). Undo-delete reconstructs the field | `frontend/src/pages/Ads.tsx`, `backend/src/routes/ads.ts`, `backend/src/database/schema.ts` |

| 78 | **Reporters: Taluka region 2026-09-02** - "Taluka" added to the Region options in the Reporters **Location** form (and shared reporter form). Region values stay lowercase on selection/display; dropdown options show title-case via `REGION_LABELS` (Local, Taluka, State, District). Task location picker region tags render lowercase too | `frontend/src/pages/Reporters.tsx`, `frontend/src/pages/Tasks.tsx` |

| 79 | **Fix: ads.brand_type missing on live Supabase 2026-09-04** - creating a Brand ad saved to the local mirror but never reached PostgreSQL because the live `ads` table (created before the Brand feature) lacked `brand_type`; the PG path relied on `CREATE TABLE IF NOT EXISTS` (a no-op on existing tables), so the column was never added. Added `ALTER TABLE ads ADD COLUMN IF NOT EXISTS brand_type TEXT` to the Postgres migration block in `initDatabase()` (mirroring the existing SQLite fallback). On next server restart the column is added to Supabase and the queued Brand-ad rows replay successfully | `backend/src/database/schema.ts` |

| 80 | **Fix: mirror-merge read gap 2026-09-04** - lists read from PostgreSQL and only merge locally-saved mirror rows when `pendingForTable(table)` > 0, but `tableOf()` matched only INSERT/UPDATE/DELETE, never SELECT, so `tableOf` returned null for list queries and pending rows were never merged — a locally-saved row that hadn't synced to PG yet (or failed to, e.g. the missing `brand_type`) stayed invisible. `tableOf()` now also extracts the table from `FROM` so list/`get` reads merge mirror-only rows and locally-saved data always shows while PG sync is pending or failing | `backend/src/database/sync.ts` |
...
| 90 | **NSIS installer script renamed to `installer/newsmeva.nsi` 2026-09-07** - `git mv` of `installer/workstation-meva-setup.nsi` → `installer/newsmeva.nsi`; header compile comment + `docs/SETUP-GUIDE-WINDOWS.md` build instructions updated. The installer OUTPUT filename stays `workstation-meva-setup.exe` (internal identifier, kept for existing deployments); historical changelog entries 68–76 referencing the old script name were left untouched | `installer/newsmeva.nsi`, `docs/SETUP-GUIDE-WINDOWS.md` |

| 91 | **Installer artifacts renamed to `newsmeva-setup.exe` + `newsmeva-online_*.deb` 2026-09-07** - Windows NSIS `OutFile` → `newsmeva-setup.exe`; `.deb` output filename → `newsmeva-online_1.0.0_amd64.deb` (new `DEB_PREFIX` in `build-deb.js`). The Debian `Package:` control field stays `workstation-meva-online` (apt upgrade identity `apt remove --purge workstation-meva-online` unchanged). Rebuilt both installers and deleted the old binaries (both git-ignored). Doc references (`SETUP-GUIDE-WINDOWS.md`, `SETUP-GUIDE-UBUNTU.md`) updated | `installer/newsmeva.nsi`, `ubuntu/installer/build-deb.js`, `docs/*` |

| 92 | **Systemd units renamed to `newsmeva.service` + `newsmeva-caddy.service` 2026-09-07** - `git mv` of `ubuntu/workstation-meva.service`, `redhat/workstation-meva.service`, `ubuntu/installer/files/workstation-meva.service`, `ubuntu/installer/files/workstation-meva-caddy.service`. All references updated: `ubuntu/install.sh`, `redhat/install.sh` (stop/disable/cp/sed/enable/journalctl), deb `postinst`/`prerm`/`postrm` (enable/restart/stop/disable), `build-deb.js` (bundles the units at `/etc/systemd/system/`, Services log line), manual launchers `start.sh`/`stop.sh` (unit + `/tmp/newsmeva-caddy.log`), `caddy-watchdog.sh` log path, README + setup guides (Ubuntu/RHEL) systemctl examples, `docs/from-scratch.md` unit map. Historical changelog entry 88 left untouched; entry 90's "output stays workstation-meva-setup.exe" note is superseded by 91 | ubuntu/redhat scripts + units, `ubuntu/installer/files/*`, `docs/*` |

| 93 | **Android identifiers renamed to `com.newsmeva` 2026-09-07** - Gradle `namespace` + `applicationId` → `com.newsmeva`, Kotlin `package` moved via `git mv` to `android/app/src/main/java/com/newsmeva/` (MainActivity.kt, NodeService.kt), intent action strings `com.newsmeva.START`/`.STOP`, theme `Theme.NewsMeva` (themes.xml + manifest), `rootProject.name = "NEWSMEVA"`, button label "Open NEWS MEVA". Note: new applicationId means devices with the old `com.workstation.meva` APK treat a fresh install as a different app | `android/app/build.gradle`, `android/settings.gradle`, `android/app/src/main/*` |

| 94 | **Contact info → newsmeva + version bump to v3.0.0 2026-09-07** - About/contact info updated app-wide: website `www.newsmeva.com`, email `info@newsmeva.com`, phone `8600633899`, address "News Meva, Maliwada, Ahilyanagar, MH IN - 414001" (Contact/FAQ/Privacy/Terms/Settings/About pages; old `marathimeva.com`/`+91 86006 33899`/`kuldeep7ke` address removed). Version bumped `1.0.0-beta.1` → `3.0.0` in `appMeta.ts` APP_VERSION, root/frontend/backend `package.json`+locks, NSIS `VIProductVersion 3.0.0.0` + welcome text, `build-deb.js` VERSION default (deb → `newsmeva-online_3.0.0_amd64.deb`), Android `versionName "3.0.0"`/`versionCode 3`, README + setup guides. Channel Metadata `website_url` seeded/defaulted to `www.newsmeva.com` (schema seed INSERTs PG+SQLite, GET fallback, Settings form default) so shared task messages include the site by default; `APP_STATUS='Beta'`/`TESTING_MODE` left unchanged | `frontend/src/*`, `backend/src/database/schema.ts`, `backend/src/routes/channelMetadata.ts`, `installer/newsmeva.nsi`, `ubuntu/installer/build-deb.js`, `android/app/build.gradle`, `package.json`(s)+locks, `README.md`, `docs/*` |

| 95 | **Supabase DB connected + online runtime + LAN hostname retargeted 2026-09-07** - `backend/.env` `DATABASE_URL` filled (Supabase pooler `aws-0-ap-northeast-2.pooler.supabase.com`, project `rklrzkwewwyctmpnesen`); server now boots itself into Postgres/online mode (verified: "[db] Initializing in postgres mode", schema initialized, 213 rows reconciled into the sync mirror, UI polling health/sync-status/dashboard OK). Channel-metadata on the EXISTING cloud DB kept as-is (News 24 Sahyadri row not overwritten - seed only applies on empty tables; operator sets branding in Settings). LAN fixes: Control Panel + `start-server.ps1` now show the friendly hostname `newsmeva` (constant, was `$env:COMPUTERNAME` = old OS name `n24s1`); server PC hosts entry `192.168.1.14 workstation` → `192.168.1.9 newsmeva` (IP was stale); all `lan/*` scripts + README `SERVER_IP` updated `192.168.1.14` → `192.168.1.9` (current machine IP); `backend/src/index.ts` comment `n24s1` → `newsmeva`. Pre-restart mirror snapshot kept at `backend/workstation.db.pre-restart.*.bak` | `backend/.env` (git-ignored), `windows/Control Panel.ps1`, `windows/start-server.ps1`, `lan/*`, `backend/src/index.ts` |

| 96 | **UI/UX brand sweep after rename 2026-09-07** - Removed all leftover old-brand strings visible in the UI: login subtitle "Workstation Tracker" → "WorkStation Online" (`Login.tsx` + README title `# WorkStation Online (NEWS MEVA)`); Settings channel-metadata website placeholder "www.workstation.com" → "www.newsmeva.com"; destructive-cleanup default-admin text `dev@workstation.local` → `dev@newsmeva.local`, profile name "Workstation Dev" → "News Meva Dev" (frontend Settings confirmations + backend `settings.ts` clean-all-data seed + message + `scripts/reset-db.js`/`reset-fixed.js`); footer "Powered by MarathiMevasa News Network" → "Powered by NEWS MEVA". Rebuilt backend+frontend, restarted server (postgres mode, schema init, serving) and rebuilt both installers (`newsmeva-setup.exe` 67.5 MB, `newsmeva-online_3.0.0_amd64.deb` 54 MB). Verified bundles: only remaining "Workstation Tracker" strings are the intentional localStorage migration guards in `appConfig.ts`/`Landing.tsx` (compare legacy saved app names, never rendered). Note: visible channel branding on landing/reading screens still comes from cloud `channel_metadata` ("News 24 Sahyadri" - kept by decision); update in Settings → Channel Metadata when ready | `frontend/src/pages/Login.tsx`, `frontend/src/pages/Settings.tsx`, `backend/src/routes/settings.ts`, `backend/scripts/*` |

| 97 | **Installer hardening: versioned exe, OS prereq check, README-after-uninstall 2026-09-09** - `installer/newsmeva.nsi` `OutFile` → `newsmeva-setup-v3.0.0.exe` (app-name + version); added `WinVer.nsh` + `x64.nsh`; install section now aborts with a clear message unless 64-bit Windows 10/11 (`${RunningX64}` + `${AtLeastWin10}`); Node.js/npx/Caddy stay bundled (fully offline install, nothing downloaded at setup); installer now also copies `README.md` to `$INSTDIR`; uninstaller opens the bundled README after uninstalling (local file survives keep-data path) and falls back to the GitHub readme page https://github.com/kuldeep7ke/newsmeva on clean-all (folder removed). Rebuilt both installers: verified `newsmeva-setup-v3.0.0.exe`~70.8 MB (version resources 3.0.0), `newsmeva-online_3.0.0_amd64.deb` 54 MB. Build toolchain now present locally: NSIS 3.12 (`C:\Program Files (x86)\NSIS\Bin\makensis.exe`), `tools/node/node-v24.19.0-win-x64/` (portable extraction), `tools/node/node-v24.19.0-linux-x64.tar.xz`. Installers + node archives are git-ignored. Setup guide `SETUP-GUIDE-WINDOWS.md` updated (new filename, system requirements check, README-after-uninstall notes) | `installer/newsmeva.nsi`, `docs/SETUP-GUIDE-WINDOWS.md`, `docs/MEMORY-CAPSULE.md` |

| 98 | **Install path rename "workstation meva" → "newsmeva" repo-wide 2026-09-09** - per user request, install-path/hostname rebrand across all platforms; intentional leftovers are legacy-cleanup or internal identifiers. Windows: NSIS `InstallDir C:\Workstation-Meva` → `C:\NewsMeva`, registry `Software\WorkstationMeva` → `Software\NewsMeva` (uninstall key `Uninstall\NewsMeva`), install-section (re)added schtasks cleanup for BOTH legacy task names ("NewsMeva" + "WorkstationMeva"). Linux: `ubuntu/installer/build-deb.js` package `workstation-meva-online` → `newsmeva-online`, install `/opt/workstation-online` → `/opt/newsmeva`, Node `/opt/workstation-node` → `/opt/newsmeva-node`; updated `ubuntu/installer/files/{postinst,postrm,newsmeva.service,newsmeva-caddy.service}`, `ubuntu/{install,start}.sh`, `ubuntu/newsmeva.service`, `redhat/{install,start}.sh`, `redhat/newsmeva.service`. Mac: LaunchAgent label `com.workstation-meva-online.server` → `com.newsmeva-online.server` in `mac/Install AutoStart.command`, `mac/Remove AutoStart.command`, `mac/Stop Server.command` (each also unloads/removes the legacy plist). LAN/proxy: stale hostname `http://workstation` → `http://newsmeva` + IP `192.168.100.156` → `192.168.1.9` in `proxy/Start Caddy.bat` + `proxy/caddy/Caddyfile` (friendly hostname is `newsmeva` per entry 95). Docs updated: `SETUP-GUIDE-WINDOWS.md`, `SETUP-GUIDE-UBUNTU.md`, `SETUP-GUIDE-RHEL.md`, `README.md`, `docs/from-scratch.md`. INTENTIONALLY KEPT (internal identifiers, matching previous decisions in entries 88/91): `package.json` name `workstation-meva-online`; harmless legacy refs in `lan/*` + `windows/Control Panel.ps1` comments ("Add Workstation Hosts.*" are real sibling filenames; "workstation" hosts-line cleanup is deliberate); historical capsule entries 72/95/etc. untouched (this entry supersedes 88/91 for INSTALL paths, but apt/npm identities remain mirror-specific). NB: existing v3.0.0 release assets still have OLD install paths — must rebuild + re-upload after this entry | `installer/newsmeva.nsi`, `ubuntu/installer/build-deb.js`, `ubuntu/installer/files/*`, `ubuntu/*`, `redhat/*`, `mac/*`, `proxy/*`, `lan/*`, `windows/*`, `docs/*` |

| 99 | **v3.0.0 -> v3.1.0 bump after immutable release lock; fresh v3.1.0 release 2026-09-09** - After entry 98 rebuild, GitHub refused to re-create the v3.0.0 tag/release (immutable-release protection persists even after the old release + remote tag were deleted; a differently-named test tag pushed fine, so the block is name-specific). User chose to bump to **v3.1.0**. Updated: NSIS `OutFile newsmeva-setup-v3.1.0.exe` + `VIProductVersion 3.1.0.0` + ProductVersion/FileVersion keys + welcome-text "BETA release (v3.1.0)"; `ubuntu/installer/build-deb.js` `VERSION` default -> `3.1.0`; docs (`SETUP-GUIDE-WINDOWS.md` v3.0.0->v3.1.0 incl all `newsmeva-setup-v3*.exe` refs; `SETUP-GUIDE-UBUNTU.md` deb filename `newsmeva-online_3.1.0_amd64.deb`; `README.md` beta/status lines). App-internal version stays `3.0.0` (`appMeta.ts` APP_VERSION, root/frontend/backend package.jsons, android `build.gradle`) - only installer/release packaging bumped. Rebuilt + verified: `newsmeva-setup-v3.1.0.exe` (installs to `C:\NewsMeva`, no stale workstation install paths in payload) and `newsmeva-online_3.1.0_amd64.deb` (`/opt/newsmeva` + `/opt/newsmeva-node`, systemd services correct, apt identity `newsmeva-online`). Local commit `e9d4b3e` pushed (entry-98 rename); then created new tag `v3.1.0` + GitHub release "NEWS MEVA Online v3.1.0 (Beta)" with both rebuilt installers. NOTE: local git tag `v3.0.0` was deleted and remote tag deleted; the old `newsmeva-setup-v3.0.0.exe`/`newsmeva-online_3.0.0_amd64.deb` builds are superseded and can be deleted locally | `installer/newsmeva.nsi`, `ubuntu/installer/build-deb.js`, `docs/SETUP-GUIDE-WINDOWS.md`, `docs/SETUP-GUIDE-UBUNTU.md`, `README.md`, `docs/MEMORY-CAPSULE.md` |
| 100 | **Re-verify pass: leftover rebrand strings + live installer smoke test 2026-09-09** - Post-release audit found + fixed 5 leftover `workstation` strings missed in entries 98/99: `ubuntu/install.sh` + `redhat/install.sh` clones-guard message ("full clone of the workstation-online repository" -> newsmeva), `docs/from-scratch.md` project-structure tree root `workstation-online/` -> `newsmeva/` + render.yaml snippet `name: workstation-meva` -> `newsmeva`, and `render.yaml` itself (`name: workstation-meva` -> `newsmeva`). Remaining `workstation` refs are all intentional (package.json npm name `workstation-meva-online`, mac `OLD_PLIST` legacy cleanup, lan/proxy legacy hostname cleanup, Control Panel/start-server comments referencing the real `lan/Add Workstation Hosts.*` filenames, historical capsule entries, NSIS legacy schtasks/firewall/shortcut cleanup lines). VERIFIED: local git = origin (0b17319), only v3.1.0 tag exists, old v3.0.0 release + tag gone both local+remote, release assets byte-identical to local builds (SHA256 match). LIVE TEST: silent-installed `newsmeva-setup-v3.1.0.exe` to temp dir -> full tree (node/node.exe v24.19.0, backend/dist, proxy/caddy/caddy.exe, README, windows launchers), zero stale install-path strings in installed payload; bundled backend BOOTED on free port 3105 and served GET /api/health -> HTTP 200 {status:ok} (in-tree server already holds 3002); silent Uninstall.exe removed all files + registry keys + firewall rule `NewsMeva 3002` + Start Menu/desktop folders (empty data dir left = keep-data default). Deleted old superseded v3.0.0 exe+deb from installer/ | `ubuntu/install.sh`, `redhat/install.sh`, `docs/from-scratch.md`, `render.yaml`, `docs/MEMORY-CAPSULE.md` |
| 101 | **Mojibake fix + final brand sweep + rebuilt installers 2026-09-09** - User report: unknown characters (mojibake glyphs like "a-circumflex + Euro + right-double-quote") in files. Byte diagnosis confirmed double-encoded UTF-8 (original UTF-8 bytes decoded as cp1252 then re-encoded; e.g. the byte sequence C3 A2 E2 82 AC E2 80 9D = an em-dash). Fixed 29 files via cp1252->UTF-8 round-trip fixer (per-line, only when round-trip is clean): backend src (schema.ts, index.ts, scripts/import-full-version.ts), frontend src (MobileApp.tsx, TaskDetail.tsx, Login.tsx subtitle `WorkStation Online` -> `News Meva Online`, Developer.tsx hostname hint -> newsmeva), android NodeService.kt, proxy/caddy/Caddyfile (em-dashes now proper), all mac/linux scripts + caddy-watchdog.sh, docs (README, from-scratch, all SETUP-GUIDEs, SETUP-SUPABASE, RESEARCH-REPORT), shell launchers. Brand sweep completed: README title `# WorkStation Online (NEWS MEVA)` -> `# NEWS MEVA`; lan helper files RENAMED `Add Workstation Hosts.bat/.command` -> `Add NewsMeva Hosts.bat/.command` (git mv; inside they already add `192.168.1.9 newsmeva`); all references updated (NSIS newsmeva.nsi bundling, README, lan/README, SETUP-GUIDE-WINDOWS incl troubleshoot + quick-ref, Control Panel.ps1 + start-server.ps1 comments, from-scratch LAN table + mac plist `com.workstation.meva` -> `com.newsmeva-online.server` + android `com.workstation.meva` package -> `com.newsmeva` + `showWorkstation()` -> `showNewsmeva()`); `cd workstation` -> `cd newsmeva`, `http://workstation` -> `http://newsmeva` in all guides; taskkill WINDOWTITLE filter -> `NEWS MEVA Server*`. KEPT intentionally: `workstation.db` runtime filename (real DB file, everywhere), npm identities (`workstation-meva-online`/`-backend`/`-frontend`), legacy-cleanup strings (NSIS/mac OLD_PLIST/lan hosts cleanup), `Workstation Meva`/`Tracker` localStorage migration guards in appConfig.ts/Landing.tsx, historical capsule entries. REBUILT clean bundles (backend tsc + frontend vite, byte-verified zero mojibake marker in dist) + both installers (`newsmeva-setup-v3.1.0.exe` 70,794,579 B, `newsmeva-online_3.1.0_amd64.deb` 56,621,724 B; deb control Package `newsmeva-online` Version `3.1.0`; payload app files mojibake-free, README title `# NEWS MEVA`, no stale install paths; only binary libs keep the byte pattern legitimately). Rebuilt backend smoke-tested: GET /api/health -> HTTP 200 {status:ok} on port 3106. v3.1.0 release assets then replaced with these rebuilt installers (see entry 102 for upload result) |
