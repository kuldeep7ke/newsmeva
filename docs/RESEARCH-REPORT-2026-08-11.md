# Research Report â€” Baseline Snapshot (2026-08-11)

> Purpose: first saved look at how NEWS MEVA is actually being used.
> This is the **baseline** â€” from this date, the app automatically collects
...
| Client-side errors / glitches (page, error, stack, user) | `telemetry_errors` table (local mirror + Supabase) | 90 days, auto-pruned | Backups â†’ Research Data |
| Every work action (who did what) | `activity_logs` (existing) | forever | Backups â†’ Research Data |
| Task workflow state changes | `task_audit_log` (existing) | forever | Backups â†’ Research Data |
| Sync engine health | `sync_log` (existing) | forever | Backups â†’ Research Data |

Download anytime: **Backups page â†’ Research Data** â€” Full report (JSON) or
...
3. **Sync churn on 2026-08-11 (400 log entries)** â€” mostly replay attempts
   during today's queue-fix work. Expected to settle; watch `Backups â†’
   Database â†’ Sync Status` (queuePending should be 0).
...
## What to improve next (when ~2â€“4 weeks of telemetry exist)

1. **Simplicity** â€” drop or auto-hide features nobody opens (usage heat).
2. **Time-saving** â€” shorten the most-used paths (defaults, one-click actions);
   spot rework loops in the workflow (the correction cycle is usually the
   biggest time loss in newsrooms).
3. **Flexibility** â€” adjust statuses/fields to the path the team actually uses.
4. **Data relevance** â€” make dashboards per-role from real usage.

## How to take the next snapshot

1. Open the app â†’ **Backups â†’ Research Data â†’ Full report (JSON)**.
