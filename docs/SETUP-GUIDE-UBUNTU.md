# NEWS MEVA Online — Ubuntu / Debian Setup Guide

Step-by-step instructions for running the **NEWS MEVA Online** server on an **Ubuntu** (or any Debian-based) machine, making it available to every user on the office LAN, auto-starting it at boot as a background service, and troubleshooting common issues.
...
A prebuilt, **fully offline** Debian/Ubuntu package is available: `installer/newsmeva-online_3.1.2_amd64.deb`.
...
```bash
sudo apt install ./newsmeva-online_3.1.2_amd64.deb
```
...
Upgrading later = `sudo apt install ./newsmeva-online_3.1.2_amd64.deb` again when a new `.deb` is
