...
A prebuilt, **fully offline** Debian/Ubuntu package is available: `installer/newsmeva-online_3.2.0_amd64.deb`.
...
```bash
sudo apt install ./newsmeva-online_3.2.0_amd64.deb
```
...
Upgrading later = `sudo apt install ./newsmeva-online_3.2.0_amd64.deb` again when a new `.deb` is
