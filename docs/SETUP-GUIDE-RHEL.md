# NEWS MEVA Online — RHEL / CentOS / Rocky / AlmaLinux / Fedora Setup Guide
...
3. [Install & Configure the Server](#3-install--configure-the-server)
...
**Quick path (recommended):** clone the repo on the RHEL machine, then run the installer:
...
6. Opens port `3003` in firewalld
...
The installer opens port `3003` in firewalld automatically. To do it manually:
...
| LAN users can't connect | Firewall: `sudo firewall-cmd --permanent --add-port=3003/tcp` then reload |
