...
#
# The app has no native modules (pg, express, socket.io, bcryptjs),
# so the same code runs on Ubuntu, Debian, RHEL, Rocky, AlmaLinux,
# CentOS Stream, and Fedora. Only the Node.js install and firewall
# commands differ â€” this script handles those (dnf + firewalld).
#
# Usage:
...
echo "============================================"
echo "  NEWS MEVA Online - RedHat Installer"
echo "============================================"
echo ""
...
if [[ "$NODE_VER" -lt 18 ]]; then
    ...
    echo "Upgrade Node.js first (see docs/SETUP-GUIDE-RHEL.md)."
    exit 1
fi
...
if command -v firewall-cmd >/dev/null 2>&1; then
    echo "Opening port $PORT for LAN users..."
    sudo firewall-cmd --permanent --add-port=$PORT/tcp
    sudo firewall-cmd --reload
    echo "Port $PORT opened."
else
    echo "firewalld not found â€” open port $PORT manually for LAN access."
fi
