# redhat/ — News Meva RHEL-family installer & launchers

**RHEL / CentOS / Rocky / AlmaLinux / Fedora** — automatic full install with
systemd (`dnf` + `firewalld`), or a manual no-systemd launcher. Server runs on
**port 3003**.
...
installs the systemd service `newsmeva`, and opens **firewalld port 3003**.
...
```bash
bash redhat/start.sh            # builds if needed, heals firewalld 3003, starts hidden (auto-restart), waits for health, opens browser
bash redhat/stop.sh             # stops server + wrapper + Caddy
```

`--foreground` keeps the classic foreground log mode. Details in
[docs/SETUP-GUIDE-RHEL.md](../docs/SETUP-GUIDE-RHEL.md).

## Files

- `install.sh` — full installer (dnf + firewalld + systemd)
