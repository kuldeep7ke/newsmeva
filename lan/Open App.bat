@echo off
title NEWS MEVA - Open App (LAN)
REM ============================================================
REM Opens the NEWS MEVA app from any office PC.
REM No admin needed, nothing to install - just a browser.
REM
REM Uses the friendly http://newsmeva when it actually works,
...
powershell -NoProfile -Command "try{ if((Invoke-WebRequest -Uri 'http://newsmeva' -UseBasicParsing -TimeoutSec 3).StatusCode -eq 200){exit 0}else{exit 1} }catch{exit 1}" >nul 2>&1
if not errorlevel 1 (
  start "" "http://newsmeva"
  exit /b 0
)
